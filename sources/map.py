...
class Map:
    ...
    def switch_map(self, map):
        ...
        self.collisions = pyscroll.PyscrollGroup(map_layer=self.map_layer, default_layer=5)
        self.changes = pyscroll.PyscrollGroup(map_layer=self.map_layer, default_layer=5)
        # Crée un groupe qui contient les hautes herbes -> quand un Pokémon sauvage peut apparaître
        self.weeds = pyscroll.PyscrollGroup(map_layer=self.map_layer, default_layer=5)
        # Crée un groupe pour les personnages
        self.pnjs = pyscroll.PyscrollGroup(map_layer=self.map_layer, default_layer=5)
